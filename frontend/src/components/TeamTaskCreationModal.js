...
import { useToast } from "@/hooks/useToast"; // Utilisation du hook useToast

const DEFAULT_AVATAR =
  "https://hebbkx1anhila5yf.public.blob.vercel-storage.com/image-L1LHIDu8Qzc1p3IctdN9zpykntVGxf.png";

// Préfixe à toujours utiliser pour les tâches de maintenance
...
export default function TaskCreationFormMaintenance({ onSuccess, onCancel, mode = "create", initialData = null }) {
  ...
  // États pour gérer le formulaire (le titre est géré par un suffixe, le préfixe étant fixe en mode maintenance)
  ...
  // Utilisation du hook de notifications (doit retourner une fonction "toast")
  const { toast } = useToast();
  const { user } = useAuth();
  const { t } = useTranslation();

  // Lors d'une édition, extraire le suffixe du titre si le préfixe est présent
  useEffect(() => {
    if (initialData) {
  ...
  const loadUsers = async () => {
    try {
      ...
      toast(t("error"), t("errorLoadingUsers"), "destructive");
    } finally {
      setLoadingUsers(false);
    }
  };

  // Fonction de compression de fichier (images, PDF et autres)
    ...
    if (files && files.length > 0) {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        try {
          ...
          toast(t("success"), `${file.name} compressed successfully`, "success");
        } catch (err) {
          console.error("Error processing file:", err);
          toast(t("error"), err.message || `Error processing ${file.name}`, "destructive");
          continue;
        }
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast(t("success"), mode === "edit" ? t("taskModified") : t("taskCreated"), "success");
      ...
      <form onSubmit={handleSubmit} className="space-y-4 md:space-y-6 w-full max-w-full px-2 sm:px-4">
        <div className="space-y-2">
          ...
          <div className="flex gap-2">
            {maintenanceEnabled && (
              <Input value={DEFAULT_PREFIX} readOnly className="w-40 bg-gray-100 dark:bg-gray-700 border-input text-foreground cursor-not-allowed" />
            )}
            <Input
              id="title"
              value={titleSuffix}
              ...
              placeholder={t("EnterTitle") || "Entrez le titre"}
            />
          </div>
        </div>
        <div className="space-y-2">
          ...
          <Textarea
            ...
            placeholder={t("EnterDescription") || "Entrez la description"}
          />
        </div>
          ...
          <div className="space-y-2">
            ...
            <Select value={priority} onValueChange={(value) => setPriority(value)}>
              <SelectTrigger className="bg-background border-input text-foreground">
                <SelectValue placeholder={t("selectPriority") || "Sélectionnez la priorité"} />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="low">{t("low") || "Basse"}</SelectItem>
                <SelectItem value="medium">{t("medium") || "Moyenne"}</SelectItem>
                <SelectItem value="high">{t("high") || "Haute"}</SelectItem>
              </SelectContent>
            </Select>
          </div>
          <div className="space-y-2">
            ...
            <Select value={status} onValueChange={(value) => setStatus(value)}>
              <SelectTrigger className="bg-background border-input text-foreground">
                <SelectValue placeholder={t("selectStatus") || "Sélectionnez le statut"} />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="todo">{t("todo") || "À faire"}</SelectItem>
                <SelectItem value="in_progress">{t("inProgress") || "En cours"}</SelectItem>
                <SelectItem value="review">{t("review") || "En révision"}</SelectItem>
                <SelectItem value="done">{t("done") || "Terminé"}</SelectItem>
              </SelectContent>
            </Select>
          </div>
        </div>
                  ...
                  {assignedTo ? (
                    <div className="flex items-center gap-2">
                      <Avatar className="h-6 w-6">
                        <AvatarImage src={users.find((u) => u._id === assignedTo)?.avatar || DEFAULT_AVATAR} alt={getUserDisplayName(users.find((u) => u._id === assignedTo))} />
                        <AvatarFallback>
                    ...
                    <SelectItem key={user._id} value={user._id}>
                      <div className="flex items-center gap-2">
                        ...
                        <div className="flex flex-col">
                          ...
                          <span className="text-xs text-muted-foreground dark:text-white/70">{user.email.split("@")[0]}</span>
                        </div>
                      </div>
                    </SelectItem>
                  ))}
        ...
        <div className="flex justify-end gap-2">
          {onCancel && (
            <Button type="button" variant="outline" onClick={onCancel} className="bg-background border-input text-foreground hover:bg-accent">
              {t("cancel")}
            </Button>
          )}
          <Button type="submit" disabled={loading} className="bg-[#B7B949] text-white">
            {loading ? (
              <>
